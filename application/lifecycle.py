"""Platform lifecycle services — workspace initialization and environment validation."""
...
def init_workspace(
    ...
    *,
    ...
    for name, path in directory_targets.items():
        action, message = _ensure_directory(path)
...
def _ensure_directory(path: Path) -> tuple[str, str]:
    try:
